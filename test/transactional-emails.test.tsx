...
const cases = [
  ...
  { name: "pending e-transfer registration", call: () => { h.read(registrations, pendingRecord); return pending(1, 2, "registration"); }, subject: "New e-transfer registration awaiting payment: ARTIS-2", to: "academy@example.com", replyTo: "guardian@example.com", text: "new registration" },
  { name: "pending e-transfer renewal", call: () => { h.read(registrations, pendingRecord); return pending(1, 2, "renewal"); }, subject: "E-transfer renewal awaiting payment: ARTIS-2", to: "academy@example.com", replyTo: "guardian@example.com", text: "training renewal" },
...
describe.each(cases)("$name sender and template", entry => {
  it("sets sender, recipient, subject and optional reply-to and renders essential content", async () => {
    ...
    if (entry.name.includes("pending e-transfer")) { for (const value of ["Awaiting e-transfer", "Test Player", "Test Guardian", "guardian@example.com", "519-555-0123", "Development", "Three months", "113.00", "ARTIS-2", "Reservation expires", "Registration number"]) { expect(message.text).toContain(value); expect(html).toContain(value); } expect(html).toContain(entry.text); }
...
describe("email validation", () => {
  ...
  it("requires pending e-transfer relationship and reference before notifying", async () => { h.read(registrations); await expect(pending(1, 2, "registration")).rejects.toThrow("could not be found"); expect(sqlQuery(h.queries(registrations)[0]).params).toEqual([1, "pending_payment", "pending", "e_transfer"]); expect(send).not.toHaveBeenCalled(); });
  it("uses contact recipient fallback for internal notification", async () => { delete process.env.E_TRANSFER_NOTIFICATION_EMAIL; h.read(registrations, pendingRecord); await pending(1, 2, "registration"); expect(send.mock.calls[0][0].to).toBe("contact@example.com"); });
