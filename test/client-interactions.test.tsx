...
const contactAction =
  jest.fn<typeof import("@/app/contact/actions").submitContactMessage>();
const renewalAction =
  jest.fn<
    typeof import("@/app/register/renew/actions").requestRenewalVerification
  >();
const cancelAction =
  jest.fn<
    typeof import("@/app/admin/registrations/actions").cancelRegistrationAction
  >();
const rescheduleAction =
  jest.fn<
    typeof import("@/app/admin/registrations/actions").rescheduleRegistrationAction
  >();
...
beforeAll(async () => {
  jest.doMock("@/app/register/actions", () => ({
    submitRegistration: registrationAction,
  }));
  jest.doMock("@/app/contact/actions", () => ({
    submitContactMessage: contactAction,
  }));
  ...
  ({ RescheduleRegistrationControl: Reschedule } =
    await import("@/app/admin/registrations/reschedule-registration-control"));
...
describe("registration client interactions with mocked actions", () => {
  it("displays field errors, red borders and summary, focuses first error and preserves entered values", async () => {
    registrationAction.mockResolvedValue({
      ...
      code: "invalid-form",
      fieldErrors: {
        childFirstName: "Enter the child first name.",
    ...
    const firstName = screen.getByLabelText(/Child’s first name/);
    const email = screen.getByLabelText(/^Email address/);
    const lastName = screen.getByLabelText(/Child’s last name/);
    fireEvent.change(lastName, { target: { value: "Synthetic Player" } });
    fireEvent.change(email, { target: { value: "invalid" } });
    await submit(container.querySelector("form")!);
    expect(
      screen.getByText("Please correct the highlighted fields below."),
    ).toBeInTheDocument();
    expect(firstName).toHaveAttribute("aria-invalid", "true");
    expect(firstName.className).toContain("border-artis-error");
    expect(firstName).toHaveFocus();
    expect(lastName).toHaveValue("Synthetic Player");
  ...
  it("keeps full and manually closed groups visible with waitlist links instead of radio inputs", () => {
    const { container } = render(
      <Program
        ...
        programPackages={[program]}
      />,
    );

    const selectableGroup = container.querySelector(
      `input[name="trainingGroupId"][value="${group.id}"]`,
    );
    const manuallyClosedInput = container.querySelector(
      `input[name="trainingGroupId"][value="${manuallyClosedGroup.id}"]`,
    );
    const capacityFullInput = container.querySelector(
      `input[name="trainingGroupId"][value="${capacityFullGroup.id}"]`,
    );
    ...
    expect(
      screen.getByRole("link", {
        name: `Join the waitlist for ${manuallyClosedGroup.displayName}`,
      }),
    ).toHaveAttribute(
      "href",
      `/register/waitlist?group=${manuallyClosedGroup.slug}`,
    );
    expect(
      screen.getByRole("link", {
        name: `Join the waitlist for ${capacityFullGroup.displayName}`,
      }),
    ).toHaveAttribute(
      "href",
      `/register/waitlist?group=${capacityFullGroup.slug}`,
    );
  });
  it("does not show a checkout form without available programs", () => {
    const { container } = render(
      <Program trainingGroups={[]} programPackages={[]} />,
    );
    expect(
      screen.getByText("Registration options are not currently available."),
    ).toBeInTheDocument();
    expect(container.querySelector("form")).toBeNull();
  });
});
describe("contact and renewal request UX", () => {
  it("shows contact errors, focuses the invalid field and preserves entered text", async () => {
    contactAction.mockResolvedValue({
      status: "error",
      code: "invalid-form",
      fieldErrors: { email: "Enter a valid email address." },
    });
    const { container } = render(<Contact defaultEnquiry="General Enquiry" />);
    ...
    expect(email).toHaveAttribute("aria-invalid", "true");
    expect(name).toHaveValue("Test Visitor");
    expect(
      screen.getByText("Enter a valid email address."),
    ).toBeInTheDocument();
  });
  it("shows successful mocked contact submission and clears values", async () => {
    contactAction.mockResolvedValue({ status: "success" });
    const { container } = render(<Contact defaultEnquiry="General Enquiry" />);
    const name = screen.getByLabelText(/Full name/);
    fireEvent.change(name, { target: { value: "Test Visitor" } });
    await submit(container.querySelector("form")!);
...
describe("navigation and admin controls", () => {
  it("provides public navigation and closes mobile menu with Escape while restoring focus", () => {
    const { container } = render(<Header />);
    const nav = screen.getByRole("navigation", { name: "Primary navigation" });
    expect(
      within(nav).getByRole("link", { name: "Contact Us" }),
  ...
  it("closes mobile menu when selecting a link", () => {
    const { container } = render(<Header />);
    const details = container.querySelector("details")!;
    details.open = true;
    ...
    fireEvent.click(within(mobile).getByRole("link", { name: "About Us" }));
    expect(details.open).toBe(false);
  });
  it("opens and cancels a cancellation dialog without submitting", () => {
    const { container } = render(
      <Cancel
        registrationId={1}
    ...
    expect(dialog).toHaveAccessibleName("Cancel Test Player’s registration?");
    fireEvent.click(
      within(dialog).getByRole("button", { name: "Keep registration" }),
    );
    expect(dialog.open).toBe(false);
    expect(cancelAction).not.toHaveBeenCalled();
  });
  it("retains cancellation success with an email failure warning", async () => {
    ...
    expect(
      screen.getByText(
        /Registration cancelled, but the guardian notification email/,
      ),
    ).toBeInTheDocument();
    expect(
      screen.getByRole("button", { name: "Registration cancelled" }),
    ).toBeDisabled();
  ...
  it("submits the selected month with a fixed registration ID and displays capacity errors", async () => {
    rescheduleAction.mockResolvedValue({
      status: "error",
      code: "training-group-full",
    });
    const { container } = render(
      <Reschedule
        registrationId={1}
        playerName="Test Player"
        currentStartsOn="2026-10-01"
        earliestStartMonth="2026-09"
        latestStartMonth="2028-09"
      />,
    );
    fireEvent.click(
      screen.getByRole("button", { name: /Change start month/i }),
    );
    const input = screen.getByLabelText("New starting month");
    fireEvent.change(input, { target: { value: "2026-11" } });
    await submit(container.querySelector("form")!);
    const formData = rescheduleAction.mock.calls[0][1];
    expect(formData.get("registrationId")).toBe("1");
    expect(formData.get("startMonth")).toBe("2026-11-01");
    expect(screen.getByRole("alert")).toHaveTextContent(
      "training group is full",
    );
